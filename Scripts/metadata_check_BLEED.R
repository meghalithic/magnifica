...
#### GET PATH -----
images.path <- "../../../../../../media/voje-lab/00C67493C6748AA4/BLEED/"
...
for(i in 1:length(image.list)){
  specimenNR[i] <- image.list[[i]][4] #BLEED
}
...
df.list$formation.manual <- "modern"
...
for(i in 1:nrow(txt.df)){
  txt.df$specimenNR[i] <- image.parse[[i]][4]
}
...
length(setdiff(df.images$fileName, txt.df$ImageName)) #"MHR.244.BLEED.456.tif" does not have a txt file
...
df.image.meta$Magnification #only keep 40
df.image.meta$AcceleratingVoltage

df.image.meta.trim <- df.image.meta[df.image.meta$Magnification == "40",]
nrow(df.image.meta.trim)
...
#write out filter csv
write.csv(df.image.meta.trim,
          "Data/BLEED.image.filter.csv",
          row.names = FALSE)
