...
for (i in 1:length(traits)){
  Fig[[i]] = ggplot(data = df)+ 
    geom_density(aes(x = log10(df[,traits[i]]), 
                     group = formation,
                     col = formation)) + 
...
#### P MATRIX ----
phen.var = lapply(form_data, function (x){ (cov(x[, 4:11]))}) #traits per colony (not variation within colony)
...
#Extend G
G_ext = lapply(G_std, function (x){ ExtendMatrix(x, ret.dim = 6)$ExtMat}) #not 8 because last eigen value (#8) was negative and second to last (#7 was zero basically), so want to ignore it 
...
pop.dist <- lapply(pop.size, function (x){mvrnorm(n = x, rep(0, 8), 
                                                  G_ext[[1]])})
pop.cv <- lapply(pop.dist, cov)
RS_result = RandomSkewers(pop.cv, G_ext[[1]])
results.RS <- Rarefaction(mean_by_colony[, 2:9], RandomSkewers, num.reps = 20) # already logged
res_melt = as.data.frame(data.matrix(melt(results.RS))) #melted dataframe

#colonies per formation
...
sample_sp = unlist(sample_sizes_G[sample_sizes_G > 28]) #more than 28 samples per thing (e.g., 28 colonies per species)
comp_sampleN=matrix(0,6,6) #calculating the smallest sample 

for (i in 1:length(sample_sp)){
  for (j in 1:length(sample_sp)){
    comp_sampleN[i,j]=min(sample_sp[i],sample_sp[j])
  }
  
}
comp_sampleN=comp_sampleN[-4,-4]
...
ggplot(data=res_melt,aes(res_melt$L1,res_melt$value^2))+geom_point()+
  geom_point(data=obs_melt,aes(obs_melt$N,obs_melt$RS), col="red")+
